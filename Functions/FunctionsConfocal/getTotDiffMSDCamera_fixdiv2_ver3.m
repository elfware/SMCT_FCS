function [msd,coeffDiffComponents,msdStd,msdN] = getTotDiffMSDCamera_fixdiv2_ver3(trajs,timeStep,pixelSize,Dimension,startPoint,endPoint)
...
for j=1:Dimension
    msd{j}=zeros(1,endPoint);
    msdStd{j}=zeros(1,endPoint);
    msdN{j}=zeros(1,endPoint);
end
    ...
    for dt = 1:endPoint
        ...
        for i=1:numtrajs
            if ~isempty(trajs{i})
                ...
                deltaCoords = [deltaCoords; tempTraj(1+dt:end,j) - tempTraj(1:end-dt,j)];
            end
        ...
        msdStdt = std(squaredDisplacement); %# std
        msdNt = numel(squaredDisplacement); %# n
        msd{j}(dt) = msdMean;
        msdStd{j}(dt)=msdStdt;
        msdN{j}(dt)=msdNt;
    end
    t=startPoint*timeStep:timeStep:endPoint*timeStep;
    tempfit=polyfit(t/1000,msd{j}(startPoint:endPoint),1);
    coeffDiffComponents(j)=tempfit(1)/2;
end
