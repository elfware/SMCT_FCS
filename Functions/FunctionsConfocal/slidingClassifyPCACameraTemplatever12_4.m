function [slidingInd,notstuck,angles,rellat,trajPrinclatent,thescores,okLength] = slidingClassifyPCACameraTemplatever12_4(Trajectories,DataSelectionRules,classparams,p)
...
%12_4 cutt off in max diffusion princ after averaging of positions
  ...
  for i=1:numtrajs
      ...
      [tempcoeff{i},thescores{i},trajPrinclatent(:,i)]=princomp(alltrajs(startInd:end,:));
      ...
      %Calculate diffusion constant with averaging, and throw away to fast
      %moving sliders
  ...
  [avgDiff,compDiff]=getDiffCVECamera(averagePos,classparams.newDeltat,classparams.scale,2);
  %[msd,compDiffMSD]=getDiffMSDCamera_fixdiv2_ver2(slidingPixelScores,newDeltat,classparams.scale,Dimension,newStartPoint,newEndPoint);
...
slidingInd=slidingInd(DxsAvg(slidingInd)<classparams.maxDiffusionsPrinc1);
% rellat=trajPrinclatent(1,:)./sum(trajPrinclatent);
% notstuck=trajPrinclatent(1,:)>classparams.minNotStuck;
