function [diff] = diffAutoCorrDiffandExpMultScales5(aVec,kVec,bExtra,kFactor,autoCorrsDiff,lagTimes,normNumber,autoCorrScaledBg1,autoCorrScaledBg2,corr1,corr2)
...
diff=zeros(1,numScales*numTimes);
 ...
 TheoryDiffCorrected=diffAutoCorrDiffMultScales3(1,kVec(1),zeros(1,numScales),kFactor,correctedDiff,lagTimes,normNumber);
 TheoryDiffCorrectedTrans=TheoryDiffCorrected';
 correctedDiffTrans=correctedDiff';
 
 diff=diff+abs(TheoryDiffCorrectedTrans-correctedDiffTrans(:));
 diff(isnan(diff))=inf;
