function [RCh1 RCh2 RChSum RCross RChPol k]=BinaryAoutoCorr_V7(DataSelectionRules,TTTRCh1Time,TTTRCh2Time,Res,points)
%  clear all
%  close all
...
[RSumcorr Lag]=xcov(TTTRCh1LinEvents+TTTRCh2LinEvents,maxpoints,'none');%unbiased
%RCh1=(Rxcorr(maxpoints+1:end)/Rxcorr(maxpoints+1))';
RChSum=(RSumcorr(maxpoints+1:end))';
%%%%%%%%%%%%%%%%%%%%%

%%Crosscorrelation between channels 
[RCrosscorr Lag]=xcov(TTTRCh1LinEvents,TTTRCh2LinEvents,maxpoints,'none');%unbiased
%RCh1=(Rxcorr(maxpoints+1:end)/Rxcorr(maxpoints+1))';
RCross=(RCrosscorr(maxpoints+1:end))';

%%Corelation of polarization signal
P=(TTTRCh1LinEvents-TTTRCh2LinEvents)./(TTTRCh1LinEvents+TTTRCh2LinEvents);
P(isnan(P))=0;
[RChPolcorr Lag]=xcov(P,maxpoints,'none');
RChPol=(RChPolcorr(maxpoints+1:end))';
