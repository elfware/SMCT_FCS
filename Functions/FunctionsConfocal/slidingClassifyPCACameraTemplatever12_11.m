function [slidingInd,notstuck,angles,rellat,trajPrinclatent,thescores,okLength,trajL,slidingClassified] = slidingClassifyPCACameraTemplatever12_11(Trajectories,DataSelectionRules,classparams,p)
...
%12_6 diffusion constant claculated with longer time (moving average)
%12_7 MSD diff coeff calculation again
%12_8 outputting values of classparams for individual slidersi in a matrix
%slidingClassified
%12_9 averagePos (with newDeltat) used to caluclate diffusioncoeff
%maxMinWindowVara added
  ...
  kWind=floor(classparams.newDeltat/classparams.deltat);
  ...
  for i=1:numtrajs
      ...
      if startInd<trajL(i)-1
      [tempcoeff{i},thescores{i},trajPrinclatent(:,i)]=pca(alltrajs(startInd:end,:));
      
        smoothAvg1=smooth(thescores{i}(:,1),kWind);
        smoothAvg2=smooth(thescores{i}(:,2),kWind);
        averagePos{i}=[];
   ...
   % [~,compDiff]=getDiffMSDCamera_fixdiv2_ver2(thescores,classparams.deltat,classparams.scale,2,classparams.msdStartPoint,classparams.msdEndPoint);
    
   %[~,compDiff]=getDiffCVECamera(averagePos,classparams.newDeltat,classparams.scale,2);
   
    [~,compDiff]=getDiffMSDCamera_fixdiv2_ver2(averagePos,classparams.newDeltat,classparams.scale,2,classparams.msdStartPoint,classparams.msdEndPoint);
...
slidingInd=slidingInd(trajPrinclatent(1,slidingInd)<classparams.maxPrincAbsVar);
slidingInd=slidingInd(minWindowVar1(1,slidingInd)>classparams.minMinWindowVar);
slidingInd=slidingInd(minWindowVar1(1,slidingInd)<classparams.maxMinWindowVar);
...
slidingInd=slidingInd(abs(meanDiffPrinc1(slidingInd))>classparams.minMeanDiff);
...
slidingInd=slidingInd(compDiff(1,slidingInd)<classparams.maxDiffusionsPrinc1);
slidingInd=slidingInd(compDiff(1,slidingInd)>classparams.minDiffusionsPrinc1);

slidingClassified=zeros(numtrajs,9);
slidingClassified(:,1)=trajPrinclatent(1,:)';
slidingClassified(:,2)=trajPrinclatent(2,:)';
slidingClassified(:,3)=angles(:)';
slidingClassified(:,4)=rellat(:)';
slidingClassified(:,5)=minWindowVar1(1,:)';
slidingClassified(:,6)=maxWindowVar1(1,:)';
slidingClassified(:,7)=abs(meanDiffPrinc1(:))';
slidingClassified(:,8)=trajL(:)';
slidingClassified(:,9)=compDiff(1,:)';
