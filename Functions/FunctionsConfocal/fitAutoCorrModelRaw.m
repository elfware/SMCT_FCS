...
function [expFitCombDiff] = fitAutoCorrModelRaw(timeWind,normNumber,kguessExpMax,kguessExpMin,numGuesses,theScales,compare,lb,ub,numBoots,numPreBoots,perc,scaleRes)
...
for i=1:numFits
    ...
    parfor boot=1:numPreBoots
        ...
        for j=1:numScales
            ...
            corr1=sum(scaleRes{theScales(j)}.combines{compare{i}(1)}.autoCorrValsAllRaw(bootInds1,:));
            corr1=corr1-corr1(end);
            corr1=corr1/abs(mean(corr1(1:normNumber)));
            
            corr2=sum(scaleRes{theScales(j)}.combines{compare{i}(2)}.autoCorrValsAllRaw(bootInds2,:));
            corr2=corr2-corr2(end);
            corr2=corr2/abs(mean(corr2(1:normNumber)));
            autoCorrsDiff(j,:)=corr1-corr2;
    ...
    for j=1:numScales
        corr1=sum(scaleRes{theScales(j)}.combines{compare{i}(1)}.autoCorrValsAllRaw);
        corr1=corr1-corr1(end);
        corr1=corr1/abs(mean(corr1(1:normNumber)));
        
        corr2=sum(scaleRes{theScales(j)}.combines{compare{i}(2)}.autoCorrValsAllRaw);
        corr2=corr2-corr2(end);
        corr2=corr2/abs(mean(corr2(1:normNumber)));
        autoCorrsDiff(j,:)=corr1-corr2;
    ...
    funDiff = @(x) weights.*diffAutoCorrDiffMultScales3(ones(1,numTerms),x(totNumParams/2+1:totNumParams),x(end)*bgAmpl,x(1:totNumParams/2),autoCorrsDiff,lagTimes,normNumber);
