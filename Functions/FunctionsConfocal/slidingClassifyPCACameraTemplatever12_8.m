function [slidingInd,notstuck,angles,rellat,trajPrinclatent,thescores,okLe
    ngth,trajL,slidingClassified] = slidingClassifyPCACameraTemplatever12_8(Trajectories,DataSelectionRules,classparams,p)
...
%12_9 averagePos (with newDeltat) used to caluclate diffusioncoeff
  ...
  for i=1:numtrajs
   ...
   %[~,compDiff]=getDiffCVECamera(averagePos,classparams.newDeltat,classparams.scale,2);
    ...
    [~,compDiff]=getDiffMSDCamera_fixdiv2_ver2(averagePos,classparams.newDeltat,classparams.scale,2,,newEndPoint);
...
slidingClassified=zeros(numel(slidingInd),9);
slidingClassified(:,1)=trajPrinclatent(1,slidingInd)';
slidingClassified(:,2)=trajPrinclatent(2,slidingInd)';
slidingClassified(:,3)=angles(slidingInd)';
slidingClassified(:,4)=rellat(slidingInd)';
slidingClassified(:,5)=minWindowVar1(1,slidingInd)';
slidingClassified(:,6)=maxWindowVar1(1,slidingInd)';
slidingClassified(:,7)=abs(meanDiffPrinc1(slidingInd))';
slidingClassified(:,8)=trajL(slidingInd)';
slidingClassified(:,9)=compDiff(1,slidingInd)';
