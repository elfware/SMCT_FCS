function [slidingInd,notstuck,angles,rellat,trajPrinclatent,thescores,okLength] = slidingClassifyPCACameraTemplatever13_2(Trajectories,DataSelectionRules,classparams,p)
...
%ver13 selectopm on maimum totaldiffusion from MSD instead
%direction
  ...
  for i=1:numtrajs
    ...
    [~,~,diffMSD]=getDiffMSDCamera_ver3(thescores,classparams.deltat,classparams.scale,2,classparams.msdStartPoint,classparams.msdEndPoint);
...
slidingInd=slidingInd(diffMSD(slidingInd)<classparams.maxDiffusion);
% rellat=trajPrinclatent(1,:)./sum(trajPrinclatent);
% notstuck=trajPrinclatent(1,:)>classparams.minNotStuck;
