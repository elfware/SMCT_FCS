...
GsquaredX=sum(repmat(GbfpXT,1,3).*repmat(GbfpX,3,1)*drhodphi,3); %This takes Gbfp(1,:)'*Gbfp(1,:) and sums over all theta and phi

GbfpYT=[];
GbfpY=Gbfp(2,:,:);  
GbfpYT(:,1,:)=conj(GbfpY);
GsquaredY=sum(repmat(GbfpYT,1,3).*repmat(GbfpY,3,1)*drhodphi,3);
