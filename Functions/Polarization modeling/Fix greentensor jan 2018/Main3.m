...
rho = linspace(0,sin(thetaMax),xSteps);
phi = linspace(0,2*pi,ySteps);
rho=rho(1:end-1)+diff(rho)/2; % Not realy right for the trapzoid method but ok for nice functions
phi=phi(1:end-1)+diff(phi)/2; % Not realy right for the trapzoid method but ok for nice functions
drhodphi=(rho(2)-rho(1))*(phi(2)-phi(1));
[rrho,pphi]=meshgrid(rho,phi);
rrhoL=transpose(rrho(:));
pphiL=transpose(pphi(:));
...
theta=asin(rrhoL);          %this is imagenary if to larg values are used in the above figure();imagesc(reshape(theta,size(xx)));colorbar
%phi=atan2(yyL,xxL);       %% figure();imagesc(reshape(phi,size(xx)));colorbar
Gbfp=greenTensorBFP_v2(pphiL,theta);
...
GbfpX=Gbfp(1,:,:);  
GbfpXT(:,1,:)=conj(GbfpX);
jacobian=repmat(reshape(rrhoL,[1 1 numel(rrhoL)]),3);
GsquaredX=sum(repmat(GbfpXT,1,3).*repmat(GbfpX,3,1)*drhodphi.*jacobian,3); %This takes Gbfp(1,:)'*Gbfp(1,:) and sums over all theta and phi

GbfpYT=[];
GbfpY=Gbfp(2,:,:);  
GbfpYT(:,1,:)=conj(GbfpY);
GsquaredY=sum(repmat(GbfpYT,1,3).*repmat(GbfpY,3,1)*drhodphi.*jacobian,3);
