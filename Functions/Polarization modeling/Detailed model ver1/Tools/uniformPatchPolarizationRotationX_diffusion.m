function [P,allMy,allTheta] = uniformPatchPolarizationRotationX_diffusion(phiCenter,thetaCenter,Fp,GsquaredX,numSamples,numRotationSteps,Dr,dt,dim)
...
%     phiTot(1+numRotationSteps*(i-1):numRotationSteps*i)=atan2((my(2)*cos(rotationAngles)-my(3)*sin(rotationAngles)),my(1));
    for j=1:numRotationSteps
        if numRotationSteps>1
            rotAngle=thetaStep*((rand()<0.5)*2-1);
        else
            rotAngle=0;
        end
        cR=cos(rotAngle);
