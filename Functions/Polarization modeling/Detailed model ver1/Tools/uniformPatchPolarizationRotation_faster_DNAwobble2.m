function  [IhExact,IvExact,I45plusExact,I45minusExact,IhDiscrete,IvDiscrete,I45plusDiscrete,I45minusDiscrete] = uniformPatchPolarizationRotation_faster_DNAwobble2(Fp,Param,FDNA)
...
for i=1:numTrajs
    ...
    %DNA is on average always centered around x-axos
    centerDNA=[1;0;0];
    cosNegPhiCenter=cos(0);
    sinNegPhiCenter=sin(0);
    
    u=rand(); %random number between 0 and 1
    v=(1-FDNA)+rand()*FDNA; % rnadom numberbetween 1-Fp and 1
    
    phi_DNA=2*pi*u;%phi_patch between [0 2pi] uniform over patch area
    ...
    theta_DNA=acos(2*v-1);  %thetapatch between [0 acos(1-2*Fp] over the patch area. this Interval is consistent with that the patch area = Fp/total surface area of spere
    %allTheta(i)=theta_patch;
    %coordinates in the patch coordiante system
    x_DNA=cos(phi_DNA)*sin(theta_DNA);
    y_DNA=sin(phi_DNA)*sin(theta_DNA);
    z_DNA=cos(theta_DNA);
    ...
    center= baseVec1*x_DNA + baseVec2*y_DNA + baseVec3*z_DNA;
    rotationAxes=centerDNA;
    ...
    cosNegPhiCenter=cos(-phi_DNA);
    sinNegPhiCenter=sin(-phi_DNA);
