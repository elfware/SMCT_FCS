function [IhExact IvExact IhDiscrete IvDiscrete]=PhotonTraceRotSim_blink_binning_newinformat_v2(Param,Dr,dt,trajLength)
...
%v2 detector off time can be considered with paramters shiftperiod and
%shifttime
...
burninIndex=round(burninTime/dt);
konBlink= Param.konBlink;%rate of turning on afer blinking
koffBlink =Param.koffBlink; %rate of going int othe dark state
shiftperiod=Param.shiftperiod;
shifttime=Param.shifttime;
...
%IhT(:,:)=Ih(1,:,:); Ih=IhT;
Ih = reshape(Ih,[size(Ih,2) size(Ih,3)]);
...
%IvT(:,:)=Iv(1,:,:); Iv=IvT;
Iv = reshape(Iv,[size(Iv,2) size(Iv,3)]);

%vProb=Iv./Isum;
%P=(Ih-Iv)./Isum;
% simulate blinking
onOrOff=zeros(numTrajs,numel(timeSteps)); %is fluorophore on or in the dark state

probOnBlink=1-exp(-konBlink*dt); %Prbabiltiy to turn on in a timestep;
probOffBlink=1-exp(-koffBlink*dt);%Probability to turn dark in a simestep
probs=[probOnBlink probOffBlink];
changes=[1 0;-1 0];
for i=1:numTrajs
    onOrOff(i,1)=1;
    for j=2:numel(timeSteps)
        r=rand();
        col=(probs(onOrOff(i,j-1)+1)<r)+1;
        row=onOrOff(i,j-1)+1;
        onOrOff(i,j)=onOrOff(i,j-1)+changes(row,col);
...
if strcmp(StringCom,'AddShutnoise')
    ...
    avgPhotonsT=reshape(avgPhotons,[size(avgPhotons,2) size(avgPhotons,3)]).*onOrOff;
 ...
 %Cut and splice data like in the experiments because of detector of time
 
if shifttime~=0
    totSteps=size(IhExact,2);
    numShifttime=round(shifttime/(dt*binning));
    
   timeStepsNew=0:dt*binning:(totSteps-1)*dt;
   endOfPeriodInd=find(diff(mod(timeStepsNew,shiftperiod+shifttime))<0);    
   startOfPeriodInd=[1 endOfPeriodInd+1];
   endOfOnWindowInd=endOfPeriodInd-numShifttime;
   endOfOnWindowInd(end+1)=numel(timeStepsNew);
   windLengths=endOfOnWindowInd-startOfPeriodInd+1;
   numStepsAfterCut=sum(windLengths);
   
   IhExactNew=zeros(numTrajs,numStepsAfterCut);
   IvExactNew=zeros(numTrajs,numStepsAfterCut);
   IhDiscreteNew=zeros(numTrajs,numStepsAfterCut);
   IvDiscreteNew=zeros(numTrajs,numStepsAfterCut);
   currInd=1;
   for i=1:numel(startOfPeriodInd)
    IhExactNew(:,currInd:currInd+windLengths(i)-1)=IhExact(:,startOfPeriodInd(i):endOfOnWindowInd(i));
    IvExactNew(:,currInd:currInd+windLengths(i)-1)=IvExact(:,startOfPeriodInd(i):endOfOnWindowInd(i));
        if strcmp(StringCom,'AddShutnoise')
            IhDiscreteNew(:,currInd:currInd+windLengths(i)-1)=IhDiscrete(:,startOfPeriodInd(i):endOfOnWindowInd(i));
            IvDiscreteNew(:,currInd:currInd+windLengths(i)-1)=IvDiscrete(:,startOfPeriodInd(i):endOfOnWindowInd(i));
        end
    currInd=currInd+windLengths(i);
   end
   
   
   IhExact=IhExactNew;
   IvExact=IvExactNew;
   IhDiscrete=IhDiscreteNew;
   IvDiscrete=IvDiscreteNew;
