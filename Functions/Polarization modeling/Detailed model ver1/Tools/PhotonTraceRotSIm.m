...
my(3,:,:)=0;
my=my.*repmat(rho,3,1,1); % The component in the xy plane is scaled with rho = sin(theta)

Ih=conj(my(1,:,:)).*sum(repmat(transpose(GsquaredX(1,:)),1,numTrajs,size(C,2)).*my,1) +...
   conj(my(2,:,:)).*sum(repmat(transpose(GsquaredX(2,:)),1,numTrajs,size(C,2)).*my,1) +...
   conj(my(3,:,:)).*sum(repmat(transpose(GsquaredX(3,:)),1,numTrajs,size(C,2)).*my,1);
IhT(:,:)=Ih(1,:,:); Ih=IhT;

Iv=conj(my(1,:,:)).*sum(repmat(transpose(GsquaredY(1,:)),1,numTrajs,size(C,2)).*my,1) +...
   conj(my(2,:,:)).*sum(repmat(transpose(GsquaredY(2,:)),1,numTrajs,size(C,2)).*my,1) +...
   conj(my(3,:,:)).*sum(repmat(transpose(GsquaredY(3,:)),1,numTrajs,size(C,2)).*my,1);
IvT(:,:)=Iv(1,:,:); Iv=IvT;
...
avgPhotons=sum(conj(my).*my,1)*averagePhotonsinStep;
